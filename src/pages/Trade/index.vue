<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div  class="address clearFix" v-for="(userAddress,index) in userAddressList" :key="index">
        <span @click="addressIndex = index" class="username" :class="{selected:addressIndex === index}">{{userAddress.consignee}}</span>
        <p>
      ...
      <div class="address clearFix">
        <span class="username " @click="payType = 0" :class="{selected:payType === 0}">在线支付</span>
        <span class="username" @click="payType = 1" :class="{selected:payType === 1}" style="margin-left:5px;">货到付款</span>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="detailArray in detailArrayList" :key="detailArray.id">
          <li>
    ...
    <div class="trade">
      <div class="price">应付金额:　<span>¥{{allMoney}}</span></div>
...
import { mapGetters } from 'vuex'
  export default {
    ...
    mounted(){
      this.getTrade(),
      this.addressIndex = this.userAddressList.reduce((prev,item,index)=>{
        if(item.isDefault === '1'){
          prev = index
        }
        return prev
    ...
    computed:{
      ...
      allMoney(){
        return this.detailArrayList.reduce((prev,item)=>{
          prev += item.skuNum * item.orderPrice
          return prev
        },0)
      },
      selectAddress(){
    ...
    .content {
      ...
      .address {
        ...
        .username::after {
          ...
          right: 0;
          bottom: 0;
          background: url(/images/payAbout/choosed.png) no-repeat;
        }
